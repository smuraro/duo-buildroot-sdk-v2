...
# Duo S patch: por default o supervisor NAO gerencia wifi neste hardware.
# Razao: o reCamera assume modelo dual-radio (wlan0=STA, wlan1=AP) e tudo
# que o supervisor faz com wifi (ifconfig wlan0 down/up + wpa_supplicant em
# wlan0; hostapd em wlan1) quebra no Duo S — so existe wlan0 (AIC8800 SDIO),
# que e gerenciado por /mnt/system/wifi-ap (hostapd nativo). Quando o
# supervisor mexe em wlan0 ele derruba o AP nativo. Funcao retorna 0 (true)
# se o supervisor PODE tocar em wifi; caso contrario, as funcoes wifi
# do main.sh viram no-op.
# Opt-in (raro): criar /mnt/data/supervisor-wifi pra liberar o supervisor
# a gerenciar wifi como no reCamera original.
_duos_supervisor_owns_wifi() {
    [ -f /mnt/data/supervisor-wifi ]
}
...
_sta_stop() {
    _duos_supervisor_owns_wifi || return 0
    _stop_pidname "wpa_supplicant"
}
_ap_stop() {
    _duos_supervisor_owns_wifi || return 0
...
function switchWiFi() {
    _duos_supervisor_owns_wifi || { echo "$STR_FAILED (wifi gerenciado por wifi-ap nativo)"; return 0; }
    echo $2 >"$CONF_WIFI"
...
function connectWiFi() {
    _duos_supervisor_owns_wifi || { echo "$STR_FAILED (wifi gerenciado por wifi-ap nativo; toque /mnt/data/supervisor-wifi pra liberar)"; return 0; }
